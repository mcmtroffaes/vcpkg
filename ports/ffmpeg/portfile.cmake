...
if(VCPKG_TARGET_IS_WINDOWS)
   list(APPEND SYSTEM_LIBRARIES cfgmgr32)
   list(APPEND SYSTEM_LIBRARIES mfplat)
   list(APPEND SYSTEM_LIBRARIES mfuuid)
   list(APPEND SYSTEM_LIBRARIES ws2_32)  # need lower case for matching
else()
...
if(VCPKG_TARGET_IS_OSX)
   list(APPEND SYSTEM_LIBRARIES AppKit)
   list(APPEND SYSTEM_LIBRARIES AudioToolbox)
   list(APPEND SYSTEM_LIBRARIES CoreAudio)
   list(APPEND SYSTEM_LIBRARIES CoreFoundation)
   list(APPEND SYSTEM_LIBRARIES CoreGraphics)
   list(APPEND SYSTEM_LIBRARIES CoreImage)
   list(APPEND SYSTEM_LIBRARIES CoreMedia)
   list(APPEND SYSTEM_LIBRARIES CoreServices)
   list(APPEND SYSTEM_LIBRARIES CoreVideo)
   list(APPEND SYSTEM_LIBRARIES Foundation)
   list(APPEND SYSTEM_LIBRARIES OpenCL)
   list(APPEND SYSTEM_LIBRARIES OpenGL)
   list(APPEND SYSTEM_LIBRARIES Security)
   list(APPEND SYSTEM_LIBRARIES VideoToolbox)
endif()
...
function(append_dependencies out)
    ...
    foreach(lib_name ${arg_NAMES})
        ...
        if(FFMPEG_DEPENDENCY_${lib_name}_${config})
            list(APPEND ${out} ${variant} "${FFMPEG_DEPENDENCY_${lib_name}_${config}}")
        else()
            if(NOT "${lib_name}" IN_LIST SYSTEM_LIBRARIES)
                message(WARNING "${lib_name} not found in ${path}; assuming system library even though not known as such")
            endif()
            find_library(FFMPEG_DEPENDENCY_${lib_name}_${config} NAMES "${lib_name}")
            list(APPEND ${out} ${variant} "${FFMPEG_DEPENDENCY_${lib_name}_${config}}")
        endif()
    endforeach()
    set("${out}" "${${out}}" PARENT_SCOPE)
...
append_dependencies(FFMPEG_DEPENDENCIES NAMES ${FFMPEG_DEPENDENCIES_RELEASE})
append_dependencies(FFMPEG_DEPENDENCIES NAMES ${FFMPEG_DEPENDENCIES_DEBUG} DEBUG)
